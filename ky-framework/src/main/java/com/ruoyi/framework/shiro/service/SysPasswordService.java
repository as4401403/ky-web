    ...
    public void validate(SysUser user, String password)
    {
        ...
        if (retryCount.incrementAndGet() > Integer.valueOf(maxRetryCount).intValue())
        {
            ...
            throw new UserPasswordRetryLimitExceedException(
                    Integer.valueOf(retryCount.incrementAndGet() - 1).intValue(), 1);
        ...
        if (!matches(user, password))
        {
            ...
            redisManager.set(true, getRedisRetryLimitKey(loginName), retryCount);
            throw new UserPasswordRetryLimitCountException(
                    Integer.valueOf(retryCount.incrementAndGet() - 1).intValue());
        }
        else
        {
